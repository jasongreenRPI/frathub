  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        success: false,
        message: 'Access denied. You can only view users in your organization.'
      });
    }
...
router.patch('/:id/role', authenticateToken, authorizeRole(['superuser']), async (req, res) => {
  try {
    const { role } = req.body;
...
router.delete('/:id', authenticateToken, authorizeRole(['superuser']), async (req, res) => {
  try {
